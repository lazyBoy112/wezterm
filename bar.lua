...
function M.setup()
  wezterm.on(
    ...
    function(tab, tabs, panes, config, hover, max_width)
      ...
      local width = tab.active_pane.width
      local num_tab = #tabs
      local min_w_tab = math.floor(width / num_tab) - 4
      local title = tab_title(tab)
      title = string.gsub(title, '%.exe$', '')
      if min_w_tab < max_w_tab then
        title = title:sub(1, title:len() - (title:len() - min_w_tab))
      elseif title:len() > max_w_tab then
